 ...
 * the lightweight SPV mode, a {@link com.google.leafcoin.store.SPVBlockStore} is the right choice.</p>
 *
 ...
 * {@link com.google.leafcoin.store.H2FullPrunedBlockStore} implements full verification, which is equivalent to the
...
private void checkDifficultyTransitions_V2(StoredBlock pindexLast, Block pblock) throws VerificationException,BlockStoreException
{
    final long				BlocksTargetSpacing			= 1* 60; // 1
	int					TimeDaySeconds				= 60 * 60 * 24;
	long				PastSecondsMin				=  (long)(TimeDaySeconds*0.01);
	long				PastSecondsMax				= (long)(TimeDaySeconds* 0.14);
	long					PastBlocksMin				= (int)(PastSecondsMin / BlocksTargetSpacing);
	long					PastBlocksMax				= (int)(PastSecondsMax / BlocksTargetSpacing);
 ...
 private void checkDifficultyTransitions_V3(StoredBlock storedPrev, Block nextBlock) throws BlockStoreException, VerificationException {
        checkState(lock.isHeldByCurrentThread());
    ...
    private void checkDifficultyTransitions(StoredBlock storedPrev, Block nextBlock) throws BlockStoreException, VerificationException {
        ...
        if (params.getId().equals(NetworkParameters.ID_TESTNET)) {
            ...
            if (storedPrev.getHeight()+1 >= 70) { DiffMode = 3; }
        }
        else {
            if (storedPrev.getHeight()+1 >= NDIFF_KGW_ENTRY_BLOCK) { DiffMode = 2; }
            if (storedPrev.getHeight()+1 >= NDIFF_DIGISHIELD_ENTRY_BLOCK) { DiffMode = 3; }
        }
        ...
        else if	(DiffMode == 3) { checkDifficultyTransitions_V3(storedPrev, nextBlock); return;}
    ...
    private void checkTestnetDifficulty(StoredBlock storedPrev, Block prev, Block next) throws VerificationException, BlockStoreException {
        ...
        // There is an integer underflow bug in leafcoin-qt that means mindiff blocks are accepted when time
        // goes backwards.
